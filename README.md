...
> **Status:** playable, and now juicy. A 9-play book over 13 routes — including deep shots (go, post, corner) and an RB screen — 3 **disguised** defensive looks you diagnose **post-snap** in a timed read window, a pass rush + sacks, interceptions on forced reads, and a 5-round game where you play **both sides of the ball** (call coverages, disguise looks, and jump routes on the rival's possessions) — wrapped in an arcade audiovisual layer (NES field, procedural 8-bit sound, an announcer, screen shake, confetti, and NBA-Jam-style callouts). The matchup math runs in two engines (JS + a Python mirror) kept in sync and guarded by a test suite.
...
- [x] Coach both sides — CPU possessions are now **real playable drives**: a named rival offense (K. Mercer's crew) runs the same engine while you call **MAN / COVER 3 / BLITZ** plus a **disguise** every down (the rival QB reads your shown look and is fooled by bluffs ~71% of the time), and you can **jump one route per play** — jumped the throw → INT/PBU chance spikes ~10×; guessed wrong → you vacated coverage and he runs free. Your defense is a new named unit (Vaughn, Kowalski & co., portraits included) that earns box-score credit by name; the rival kicks real FGs and punts on 4th; balance holds the old ~20 pts/game baseline so good defense is pure upside
