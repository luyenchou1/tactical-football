...
- **Sound:** **multichannel 8-bit SFX** — each event layers a tonal body + a noise transient + (on impacts) a raw sub thump, synthesized with [ZzFX](https://github.com/KilledByAPixel/ZzFX) + Web Audio — plus a **dynamic procedural crowd** (filtered noise through the ZzFX audio graph) that *swells* as a deep ball hangs, *erupts* on a score, and *groans* on a pick — over an ascending **touchdown fanfare**, a big-play riser, and a first-down ding. A `speechSynthesis` **announcer** calls the big beats ("Touchdown!", "Picked off!", "Big gainer!"), and a gentle menu-music loop ducks for the snap. A 🔊/🔇 toggle; mute is remembered. *(The announcer is wired so recorded VO clips can drop in later.)*
...
- [x] Multichannel SFX pass — every event re-synthesized as layered voices (tonal body + noise transient + sub thump), arpeggiated rewards, and raw-WebAudio impacts, from a retro-arcade SFX research sprint
